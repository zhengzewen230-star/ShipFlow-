...
async function loadList() {
  ...
  try {
    ...
    else if (domain === 'audit') data = auth.scope === 'PLATFORM' ? (tenantId.value ? await audit.listPlatformTenantAuditLogs(tenantId.value) : { items: [] }) : await audit.listTenantAuditLogs()
    else return
...
async function runOrderAction(kind: 'submit' | 'cancel' | 'inbound' | 'measure' | 'outbound' | 'price-request' | 'price-confirm') {
  if (kind === 'cancel' && !window.confirm('确认取消该订单？取消后不能恢复，请确认后继续。')) return
  if (kind === 'price-confirm' && !window.confirm('确认最终确认费用？确认后订单将进入待出库流程。')) return
  if (kind === 'outbound' && !window.confirm('确认完成出库？该操作会推进订单状态，请确认后继续。')) return
  await submit.submit(async () => {
    successMessage.value = ''
...
function previousQuotePage() { if (quoteFilter.page > 1) syncQuoteRoute(quoteFilter.page - 1) }
function nextQuotePage() { if (quoteFilter.page < quotePage.value.totalPages) syncQuoteRoute(quoteFilter.page + 1) }
...
function previousOrderPage() { if (orderFilter.page > 1) syncOrderRoute(orderFilter.page - 1) }
function nextOrderPage() { if (orderFilter.page < orderPage.value.totalPages) syncOrderRoute(orderFilter.page + 1) }
...
<template>
  <section>
    ...
    <div v-if="(domain === 'orders' || domain === 'tracking') && selected" class="panel tracking-panel">
      ...
      <div v-if="trackingLoading" class="data-state">正在加载订单轨迹…</div>
      <DataState :loading="trackingLoading" :error="trackingError" :trace-id="trackingErrorTraceId" :retry="retryOrderTracking" :empty="!trackingEvents.length" empty-title="暂无轨迹事件">
        <div class="data-table-wrap"><table class="data-table"><thead><tr><th>时间</th><th>物流单号</th><th>事件编号</th><th>事件类型</th><th>说明</th><th>处理状态</th></tr></thead><tbody><tr v-for="event in trackingEvents" :key="event.id"><td>{{ format('eventTime', event.eventTime) }}</td><td>{{ event.trackingNo }}</td><td>{{ event.eventId }}</td><td>{{ event.eventCode }}</td><td>{{ event.eventDescription || '-' }}</td><td>{{ format('processStatus', event.processStatus) }}</td></tr></tbody></table></div>
      </DataState>
    </div>
    ...
    <DataState :loading="loading" :error="error" :trace-id="errorTraceId" :retry="retryRequest" :empty="!rows.length && !selected">
      ...
      <div v-if="currentQuote" class="quote-detail">
        <div class="panel quote-detail__hero"><div><span class="kicker">正式报价</span><h2>{{ currentQuote.quoteNo }}</h2><p>以下费用和计算依据均为创建报价时锁定的规则快照。</p></div><div class="quote-detail__amount"><small>报价金额</small><strong>{{ currentQuote.currency }} {{ currentQuote.amount }}</strong><span>{{ format('status', currentQuote.status) }}</span></div></div>
        ...
        <div v-if="createdOrder" class="panel quote-order-created"><CheckCircle2 :size="20" /><div><h2>订单已创建</h2><p>订单编号：{{ createdOrder.orderNo }}，当前为草稿状态。</p></div><RouterLink class="btn btn--primary" :to="`/app/orders?orderId=${createdOrder.id}`">查看订单</RouterLink></div>
      </div>
      <div v-else-if="selected" class="panel detail-grid"><div v-for="(value, key) in selected" :key="key"><small>{{ displayLabel(String(key)) }}</small><span>{{ format(String(key), value) }}</span></div></div>
      <div v-else class="panel table-panel"><div class="data-table-wrap"><table class="data-table"><thead><tr><th v-if="domain === 'orders'"><input type="checkbox" :checked="rows.length > 0 && selectedOrderIds.length === rows.length" aria-label="全选订单" @change="toggleAllOrders" /></th><th v-for="key in columns" :key="key">{{ displayLabel(key) }}</th><th v-if="['quotes', 'orders', 'warehouse'].includes(domain)">操作</th></tr></thead><tbody><tr v-for="row in rows" :key="String(row.id)"><td v-if="domain === 'orders'"><input v-model="selectedOrderIds" type="checkbox" :value="String(row.id)" :aria-label="`选择订单 ${row.orderNo ?? row.id}`" /></td><td v-for="key in columns" :key="key">{{ format(key, row[key]) }}</td><td v-if="domain === 'quotes'"><button class="table-link" @click="selectQuote(row)">查看详情<ChevronRight :size="15" /></button></td><td v-else-if="domain === 'orders' || domain === 'warehouse'"><button class="table-link" @click="selectOrder(row)">查看详情<ChevronRight :size="15" /></button></td></tr></tbody></table></div></div>
    </DataState>
    <div v-if="domain === 'orders' && !selected && orderPage.totalPages > 1" class="page-actions quote-pagination"><button class="btn btn--secondary" type="button" :disabled="loading || orderFilter.page <= 1" @click="previousOrderPage">上一页</button><span>第 {{ orderPage.page }} / {{ orderPage.totalPages }} 页，共 {{ orderPage.total }} 条</span><button class="btn btn--secondary" type="button" :disabled="loading || orderFilter.page >= orderPage.totalPages" @click="nextOrderPage">下一页</button></div>
    <div v-if="domain === 'quotes' && !selected && quotePage.totalPages > 1" class="page-actions quote-pagination">
      <button class="btn btn--secondary" type="button" :disabled="loading || quoteFilter.page <= 1" @click="previousQuotePage">上一页</button>
      <span>第 {{ quotePage.page }} / {{ quotePage.totalPages }} 页，共 {{ quotePage.total }} 条</span>
      <button class="btn btn--secondary" type="button" :disabled="loading || quoteFilter.page >= quotePage.totalPages" @click="nextQuotePage">下一页</button>
    </div>
  </section>
</template>
