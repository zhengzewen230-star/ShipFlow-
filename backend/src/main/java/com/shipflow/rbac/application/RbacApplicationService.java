...
public class RbacApplicationService {
    ...
    public Role get(Long t,Long id){return withPermissions(t,required(t,id));}
    ...
    @Transactional public Role replace(Long t,Long id,RolePermissionBindingRequest r,Long op,String req){
        Role role=required(t,id);List<Long> ids=r.permissionIds().stream().distinct().toList();if(permissionsCount(ids)!=ids.size())throw error("COMMON-1006",404);
