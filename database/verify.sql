...
SELECT 'table_count' AS check_name, COUNT(*) AS actual_value, 28 AS expected_value,
...
SELECT 'table_collation' AS check_name, COUNT(*) AS total_tables,
       ...
       CASE WHEN COUNT(*) = 28 AND COALESCE(SUM(table_collation <> 'utf8mb4_0900_ai_ci'), 0) = 0 THEN 'PASS' ELSE 'FAIL' END AS result
FROM information_schema.tables WHERE table_schema = DATABASE() AND table_type = 'BASE TABLE';
...
SELECT 'required_unique_indexes' AS check_name, COUNT(DISTINCT index_name) AS actual_value, 15 AS expected_value,
       CASE WHEN COUNT(DISTINCT index_name) = 15 THEN 'PASS' ELSE 'FAIL' END AS result
