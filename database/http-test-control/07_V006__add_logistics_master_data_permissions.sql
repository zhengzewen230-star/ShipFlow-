-- ShipFlow shipflow_test controlled copy of V006.
-- The current 01 baseline already has logistics_channel.transport_mode;
-- this file applies only V006's additive permission data.
...
SELECT r.id, p.id FROM sys_role r JOIN sys_permission p
  ON p.permission_code IN ('logistics:read', 'logistics:manage', 'price-rule:manage')
WHERE r.role_scope = 'PLATFORM' AND r.role_code = 'PLATFORM_ADMIN'
  AND r.tenant_id IS NULL AND r.deleted = 0 AND r.status = 'ACTIVE'
  AND NOT EXISTS (SELECT 1 FROM sys_role_permission rp WHERE rp.role_id = r.id AND rp.permission_id = p.id);
INSERT INTO sys_role_permission (role_id, permission_id)
SELECT r.id, p.id FROM sys_role r JOIN sys_permission p ON p.permission_code = 'logistics:read'
WHERE r.role_scope = 'TENANT' AND r.deleted = 0 AND r.status = 'ACTIVE'
  AND NOT EXISTS (SELECT 1 FROM sys_role_permission rp WHERE rp.role_id = r.id AND rp.permission_id = p.id);
